...
const replayButton = document.getElementById('graphReplay');
let replay = null;

const startReplay = () => {
  if (!nodeMeshes.size) return;
  if (nodeInput.disabled && !replay) return;
  nodeInput.disabled = true;
  nodeSend.disabled = true;
  const ordered = [...nodeMeshes.values()].sort((a, b) =>
    a.userData.depth - b.userData.depth || a.userData.index - b.userData.index);
  const spacing = Math.min(700, Math.max(35, 14000 / ordered.length));
  const ranks = new Map(ordered.map((mesh, index) => [mesh.userData.id, index]));
  const reducedMotion = matchMedia('(prefers-reduced-motion: reduce)').matches;
  replay = { start: performance.now(), spacing, ranks, fade: reducedMotion ? 80 : 500 };
  nodeMeshes.forEach((mesh) => {
    const palette = getNodePalette(mesh.userData.depth, mesh.userData.cluster);
    mesh.material.color.setHex(palette.color);
    mesh.material.emissive.setHex(palette.emissive);
    mesh.material.emissiveIntensity = palette.emissiveIntensity;
    mesh.material.opacity = palette.opacity;
    mesh.userData.targetScale = 1;
    mesh.visible = false;
    mesh.scale.setScalar(0);
  });
  pulseGroup.children.forEach((sprite) => { sprite.visible = false; });
  edgeGroup.children.forEach((line) => { line.geometry.setDrawRange(0, 0); });
  replayButton.classList.add('is-playing');
  replayButton.setAttribute('aria-label', 'Reiniciar la formación del grafo');
  replayButton.title = 'Reiniciar la formación del grafo';
};

const updateReplay = (time) => {
  if (!replay) return;
  const elapsed = time - replay.start - 200;
  const progress = (id) => clamp((elapsed - replay.ranks.get(id) * replay.spacing) / replay.fade, 0, 1);
  nodeMeshes.forEach((mesh, id) => {
    const p = progress(id);
    mesh.visible = p > 0;
    mesh.scale.setScalar(mesh.userData.baseScale * (1 - Math.pow(1 - p, 3)));
  });
  pulseGroup.children.forEach((sprite) => {
    const p = progress(sprite.userData.id);
    sprite.visible = p > 0;
    sprite.material.opacity *= p;
  });
  edgeGroup.children.forEach((line) => {
    const positions = line.geometry.attributes.position;
    const original = line.userData.fullPositions;
    line.userData.connections.forEach((edge, index) => {
      const rank = Math.max(replay.ranks.get(edge.source), replay.ranks.get(edge.target));
      const p = clamp((elapsed - rank * replay.spacing - replay.fade * .4) / replay.fade, 0, 1);
      const offset = index * 6;
      for (let axis = 0; axis < 3; axis++) {
        positions.array[offset + axis] = original[offset + axis];
        positions.array[offset + 3 + axis] = original[offset + axis] +
          (original[offset + 3 + axis] - original[offset + axis]) * p;
      }
    });
    positions.needsUpdate = true;
    line.geometry.setDrawRange(0, positions.count);
  });
  if (elapsed >= (replay.ranks.size - 1) * replay.spacing + replay.fade * 1.4) {
    replay = null;
    replayButton.classList.remove('is-playing');
    replayButton.setAttribute('aria-label', 'Reproducir la formación del grafo');
    replayButton.title = 'Reproducir la formación del grafo';
    nodeInput.disabled = false;
    nodeSend.disabled = false;
    setSelectedNode(selectedId, { focus: false, preserveDraft: true });
  }
};
...
const nodeConnector = document.getElementById('nodeConnector');
...
const setSelectedNode = (id, { focus = true, preserveDraft = false } = {}) => {
  selectedId = id;
  ...
  if (!preserveDraft) nodeInput.value = '';
  if (focus) nodeInput.focus({ preventScroll: true });
};

const updateNodeConnector = () => {
  const mesh = nodeMeshes.get(selectedId);
  if (!mesh || !mesh.visible || nodePanel.hidden) {
    nodeConnector.setAttribute('d', '');
    return;
  }
  mesh.getWorldPosition(connectorPosition);
  connectorPosition.project(camera);
  if (connectorPosition.z < -1 || connectorPosition.z > 1 || Math.abs(connectorPosition.x) > 1 || Math.abs(connectorPosition.y) > 1) {
    nodeConnector.setAttribute('d', '');
    return;
  }
  const x = (connectorPosition.x + 1) * innerWidth / 2;
  const y = (1 - connectorPosition.y) * innerHeight / 2;
  const panel = nodePanel.getBoundingClientRect();
  const mobile = innerWidth <= 760;
  const endX = mobile ? clamp(x, panel.left + 20, panel.right - 20) : panel.left;
  const endY = mobile ? panel.top : panel.top + panel.height / 2;
  nodeConnector.setAttribute('d', mobile
    ? `M${x} ${y} L${endX} ${endY - 20} V${endY}`
    : `M${x} ${y} L${endX - 24} ${endY + 14} L${endX} ${endY}`);
};

const refreshScene = (nextGraph) => {
  const wasReplaying = Boolean(replay);
  ...
  graph.edges.forEach((edge) => {
    ...
    (points === lineageEdgePoints ? lineageConnections : crossConnections).push(edge);
  ...
  edgeGroup.children.forEach((line, index) => {
    line.userData.connections = index === 0 ? lineageConnections : crossConnections;
    line.userData.fullPositions = line.geometry.attributes.position.array.slice();
  });
  ...
  renderNodes.forEach((node, index) => {
    ...
    sprite.userData = { baseOpacity: palette.glowOpacity, id: node.id };
    pulseGroup.add(sprite);
  });

  replayButton.disabled = nodeMeshes.size === 0;
  if (wasReplaying) startReplay();
  if (selectedId && nodeMeshes.has(selectedId)) {
    setSelectedNode(selectedId, { focus: false, preserveDraft: true });
  } else {
    const firstId = nodeMeshes.keys().next().value;
    if (firstId) setSelectedNode(firstId, { focus: false, preserveDraft: true });
    else {
      selectedId = null;
...
const handleSubmit = async (event) => {
  event.preventDefault();
  if (replay || nodeInput.disabled) return;
